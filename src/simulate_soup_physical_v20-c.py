# -*- coding: utf-8 -*-
"""
Created on Sat Mar 14 12:03:18 2026
...
# -*- coding: utf-8 -*-
"""
simulate_soup_physical_v20-c.py

VERSIÓN CON SNAPSHOTS TEMPORALES Y MÁS RÉPLICAS
- Guarda snapshots cada 50 fusiones para análisis de evolución
- 5 réplicas por punto (en lugar de 3)
- Estudia escalado: D(N) vs N
- Mantiene toda la funcionalidad original

Siguiente: -> analyze_temporal_evolution.py
"""
...
os.makedirs(os.path.join(output_dir, "snapshots"), exist_ok=True)  # Directorio para snapshots
...
def save_snapshot(mol, step, fusion_count, output_dir, params):
    """Guarda un snapshot del grafo en este momento de la evolución"""
    if mol.N < 100:  # No guardar grafos muy pequeños
        return
    
    # Crear nombre con step, fusion_count y tamaño
    filename = f"snapshot_fusion{fusion_count:06d}_step{step:06d}_N{mol.N}_{params}.npz"
    filepath = os.path.join(output_dir, "snapshots", filename)
    
    save_npz(filepath, mol.adj_matrix)
    print(f"      📸 Snapshot guardado: fusion={fusion_count}, N={mol.N}")
...
def run_simulation(coupling_strength, k_sigma, k_ipr, k_gap, T, seed, initial_atoms=None):
    ...
    params_str = f"k{int(k_sigma)}_{int(k_ipr)}_{int(k_gap)}_T{int(T)}_l{int(coupling_strength*100):03d}"
    SNAPSHOT_FREQUENCY = 50  # Cada 50 fusiones
    ...
    for step in range(max_steps):
        ...
        if random.random() < P:
            ...
            if fusion_count > 0 and fusion_count % SNAPSHOT_FREQUENCY == 0:
                save_snapshot(new_mol, step, fusion_count, output_dir, params_str)
            # ============================
    ...
    if population_size == 1:  # Solo si hay un cluster
        ...
        # Guardar también snapshot final
        save_snapshot(largest_mol, step, fusion_count, output_dir, params_str)
...
print("\n" + "="*70)
print(" ESTUDIO DE ESCALADO CON SNAPSHOTS - VERSIÓN MEJORADA")
print("="*70)
print(" ✓ Genera grafos de DIFERENTES TAMAÑOS iniciales")
print(" ✓ Guarda SNAPSHOTS cada 50 fusiones para análisis temporal")
print(" ✓ 5 RÉPLICAS por punto (mayor fiabilidad estadística)")
print(" ✓ Para estudiar escalado: D(N) vs N y evolución temporal")
...
initial_sizes = [200, 500, 1000, 2000, 5000, 8480, 20000]
replicas = 5  # AUMENTADO DE 3 A 5 PARA MAYOR FIABILIDAD
...
print(f"   Réplicas por tamaño: {replicas} (↑ más robusto)")
print(f"   Snapshots cada: 50 fusiones")
...
if scaling_results:
    ...
    print("="*70)
    print(f"   📁 {output_dir}/")
    print(f"      ├── giant_graph_*_Ninit*.npz  (grafos finales)")
    print(f"      ├── snapshots/                (serie temporal)")
    print(f"      │   └── snapshot_fusion*.npz  (cada 50 fusiones)")
    print(f"      └── scaling_results_final.csv (resultados numéricos)")
    
    print("\n" + "="*70)
    print(" SIGUIENTE PASO:")
    print("="*70)
    print("""
    Ejecuta ahora el análisis temporal:
    
    python analyze_temporal_evolution.py
    
    Esto generará las gráficas de evolución:
    - Crecimiento del tamaño
    - Saturación del diámetro
    - Evolución del clustering
    - Aparición de hubs
    """)
